...
switch($function)
{
	...
	case "transDetails":
		...
		if(count($r) > 0){
			...
			for($i = 0; $i < count($r); $i++){
				echo "
				<tr>
					<td>". (($r[$i]->IsFile) ? "<a href='$_SERVER[SERVER_ROOT]/npi/files/". $r[$i]->FileLocation . "' target='_blank' title='".$r[$i]->FileName."'>". substr($r[$i]->FileName, 0, 15) . "...</a>": "Not available") . "</td>
					<td>". $r[$i]->Comments."</td>
	...
	case "itemDetails":
		...
		<div class="col-md-10">
			...
			<a href="javascript:void(0);" class="btn btn-primary" id="btnItemDetailSave">Save Changes</a>
